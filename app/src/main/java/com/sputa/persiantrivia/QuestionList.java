...
public class QuestionList extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //}
            ...
            search_type="get_question_list&typ="+list_type;
    ...
    private class MyAsyncTask extends AsyncTask<String, Integer, Double> {
        ...
        protected void onPostExecute(Double result) {
            ...
            if(end>0 && ss.length()>0) {
                ...
                if (param_str.equals("get_question_list") && is_requested) {
                    ...
                    if(end1>0) {
                        ...
                        if (param_str.length() > 0) {
                            ...
                            for (int i = 0; i < cnt; i++) {
                                ...
                                start1 = lst_item.indexOf("<question>");
                                end1 = lst_item.indexOf("</question>");
                                String
                                        question = lst_item.substring(start1 + 10, end1);
                                if(question.length()>50)
                                {
                                    question = question.substring(0,50);
                                }

                                start1 = lst_item.indexOf("<subject>");
                                end1 = lst_item.indexOf("</subject>");
                                String subject = lst_item.substring(start1 + 9, end1);


                                start1 = lst_item.indexOf("<desc>");
                                end1 = lst_item.indexOf("</desc>");
                                String
                                        desc = lst_item.substring(start1 + 6, end1);


                                start1 = lst_item.indexOf("<q_id>");
                                end1 = lst_item.indexOf("</q_id>");
                                String q_id = lst_item.substring(start1 + 6, end1);


                                user_name[i] = subject;
                                user_desc[i] = question;
                                user_level[i] = desc;
                                uig_id[i] = q_id;
                            ...
                            lv.setAdapter(new Question_adapter(QuestionList.this, user_name, user_desc, user_level,uig_id, mydatabase,list_type));
                        }
                    }
                    else
                    {
